function [CASOS1,Cs,g] = Mex_hospG_edad(Mex_dash,a,b,A)
...
if a==60
    g=plot(td(1:1:(length(td)-A)),CASOS1(1:1:(length(CASOS1)-A)),'Color',[0, 0, 1],'LineWidth',2);
elseif a==50
     g=plot(td(1:1:(length(td)-A)),CASOS1(1:1:(length(CASOS1)-A)),'Color',[0, 0.5, 0],'LineWidth',2);
elseif a==40
     g=plot(td(1:1:(length(td)-A)),CASOS1(1:1:(length(CASOS1)-A)),'Color',[1, 0, 0],'LineWidth',1.5);
elseif a==30
     g=plot(td(1:1:(length(td)-A)),CASOS1(1:1:(length(CASOS1)-A)),'Color',[0, 0.75, 0.75],'LineWidth',1.5);
elseif a==15
    g=plot(td(1:1:(length(td)-A)),CASOS1(1:1:(length(CASOS1)-A)),'Color',[0.75, 0, 0.75],'LineWidth',1.5);
elseif a==0
    g=plot(td(1:1:(length(td)-A)),CASOS1(1:1:(length(CASOS1)-A)),'Color',[0.25, 0.25, 0.25],'LineWidth',1.5);
end
tss = td(1);
tff =td(length(td)-A);
xlim([tss tff]);
title('Hospitalizaciones Semanales | Camas Generales | Edad | COVID-19 México')
