function [CASOS2,Cs,v] = Mex_hospV_edad(Mex_dash,a,b,A)
...
if a==60
    v=plot(td(1:1:(length(td)-A)),CASOS2(1:1:(length(CASOS2)-A)),'Color',[0, 0, 1],'LineWidth',2);
elseif a==50
     v=plot(td(1:1:(length(td)-A)),CASOS2(1:1:(length(CASOS2)-A)),'Color',[0, 0.5, 0],'LineWidth',2);
elseif a==40
     v=plot(td(1:1:(length(td)-A)),CASOS2(1:1:(length(CASOS2)-A)),'Color',[1, 0, 0],'LineWidth',1.5);
elseif a==30
     v=plot(td(1:1:(length(td)-A)),CASOS2(1:1:(length(CASOS2)-A)),'Color',[0, 0.75, 0.75],'LineWidth',1.5);
elseif a==15
    v=plot(td(1:1:(length(td)-A)),CASOS2(1:1:(length(CASOS2)-A)),'Color',[0.75, 0, 0.75],'LineWidth',1.5);
elseif a==0
    v=plot(td(1:1:(length(td)-A)),CASOS2(1:1:(length(CASOS2)-A)),'Color',[0.25, 0.25, 0.25],'LineWidth',1.5);
end
tss = td(1);
tff =td(length(td)-A);
xlim([tss tff]);
title('Hospitalizaciones Semanales | Camas con Ventilador | Edad | COVID-19 México')
